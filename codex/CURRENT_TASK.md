...
M30 — **Starroom v1.0.0-rc.2 field-validation fixes (feature freeze)**.
...
Produce a verifiable Windows `v1.0.0-rc.2`, not Final, from the qualified rc.1 baseline. Close the
Library, interactive Preview, true high-resolution viewport, final UI and AI packaging field issues,
then repeat complete Level-4 and installer/runtime qualification on one immutable candidate SHA.
...
The active field blocker ledger is `docs/38_RC2_FIELD_VALIDATION.md`.
...
- Do not tag or publish `v1.0.0-rc.2` until every same-SHA release gate is green. Do not declare Final, merge
...
100k/100MP plan and M28 performance regression gates, plus measured rc.2 Library/Preview/viewport
performance. Record executable/installer/model hashes and CI URLs. Only then create
`v1.0.0-rc.2` and stop for field validation.
